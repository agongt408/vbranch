...
def get_baseline_acc_from_file(params, dirpath, metric, epoch):
    ...
    for f in glob(dirpath + '/train_*'):
        results = read_json(f)
        acc_list.append(results[metric][epoch])
...
#         acc_list = results['before_mean_acc']
#         assert len(acc_list) > 0, params
    ...
    for b in branches:
        ...
        for s in shared_frac:
            ...
            for f in glob(path + '/train_*'):
                results = read_json(f)
                acc_list.append(results[f'{metric}_ensemble'][epoch])
...
if __name__ == '__main__':
    ...
    #         f'B{b}'), metric=args.metric, epoch=args.epoch-1)
