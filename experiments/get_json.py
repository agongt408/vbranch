...
num_branches = range(2, 5)

param1 = [0.4, 0.6, 0.8, 1.0]
param2 = None # [8, 16, 32, 64, 128]

def _get_results_path(dataset, arch, *params, vb=False):
    # Set kwargs manually before running
    dirpath = get_path(dataset, arch, 'results', 'data_exp-2', vb=vb, Bg=params[0])
    return dirpath

def get_baseline_acc_from_file(dataset, arch, *params):
    dirpath = _get_results_path(dataset, arch, *params)
    print(dirpath)
    ...
    assert len(acc_list) > 0, '{} {}'.format(*params)
    return np.mean(acc_list), np.std(acc_list)

def get_ensemble_acc_from_file(dataset, arch, *params, branches):
    dirpath = _get_results_path(dataset, arch, *params)
    ...
    for b in branches:
        ...
        assert len(acc_list) > 0, '{} {} {}'.format(*params, b)
        ensemble_results[b] = [np.mean(acc_list), np.std(acc_list)]

    return ensemble_results

def get_vbranch_acc_from_file(dataset, arch, *params, branches, shared_frac):
    dirpath = _get_results_path(dataset, arch, *params, vb=True)
    ...
    for b in branches:
        ...
        for s in shared_frac:
            acc_list = []
            path = os.path.join(dirpath,'B%d'%b,f'S{s:.2f}')
            for f in glob(path + '/train_*'):
                csv = pd.read_csv(f)
                last_row = csv.iloc[-1]
                acc_list.append(last_row['val_acc_ensemble'])

            assert len(acc_list) > 0, '{} {} {} {}'.format(*params, b, s)
            vbranch_results[b][s] = \
...
if __name__ == '__main__':
    ...
    for p1 in param1:
        if param2 = None:
            baseline_results[p1] = \
                get_baseline_acc_from_file(dataset, architecture, p1)
        else:
            baseline_results[p1] = {}
            for p2 in param2:
    ...
    for p1 in param1:
        if param2 is None:
            ensemble_results[p1] = \
                get_ensemble_acc_from_file(dataset, architecture, p1, num_branches)
        else:
            ensemble_results[p1] = {}
            for p2 in param2:
