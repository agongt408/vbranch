...
import evaluation,data, training
import models
...
def get_parser():
    parser = gp(
        description='Train MergeNet (https://arxiv.org/pdf/1803.05872.pdf)',
        default_file_root='mergenet')
    
    parser.add_argument('--branches', dest='branches', action='store',
                        default=3, const=3, nargs='?', type=int,
                        help='number of virtual branches')
    parser.add_argument('--overlap', dest='overlap', action='store',
                        default=0, const=0, nargs='?', type=float,
                        help='degree of overlap')
    '''
    parser.add_argument('--nth_pool', dest='nth_pool', action='store',
                        default=3, const=3, nargs='?', type=int,
                        help='nth pool to start branching')'''
    return parser
...
def get_training_params_from_parser_args(args):
    ...
    #Determine file root
    file_root = args.file_root + '_' + \
                'b' + str(args.branches) + \
                'r' + str(args.overlap) + '_' + \
                'P%dK%d' % (args.P_param, args.K_param) + '_' + \
                'pool' + str(args.nth_pool)
...
if args.test is not None:
    # Test model
    json_path = os.path.join(MODELS_ROOT, args.test[0], 
                    args.test[0] + '_report.json')
                    
    if os.path.exists(json_path):
        params, rank, mAP, loss = get_training_report_from_json(args.test[0])
    else:
        rank, mAP, loss = {}, {}, []
    ...
    model_eval = models.DenseNetDrop(
        branches=params['branches'], overlap=params['overlap'], 
    ...
    for it in args.test[1:]:
        ...
        if os.path.exists(path):
            ...
            model_eval.set_weights(np.load(path))
            s = evaluation.get_score(
                model_eval, dataset=params['dataset'], inputs=1)
            print s
    ...
    if args.continue_from_root is None:
        model = models.DenseNetDrop(
            params['P'], params['K'], branches=params['branches'], 
            overlap=params['overlap'], weights=params['weights'], 
            diagnostic=params['batch_norm_diagnostic'], 
            blocks=params['blocks'])
        if params['eval_era'] > 0:
            model_eval = models.DenseNet121Drop(
                branches=params['branches'], overlap=params['overlap'], 
                blocks=params['blocks'])
    else:
        model = models.DenseNetDrop(
            params['P'], params['K'], branches=params['branches'], 
            overlap=params['overlap'], weights=None, 
            diagnostic=params['batch_norm_diagnostic'], 
            blocks=params['blocks'])
    ...
    for era in range(params['start'],params['era'] + 1):
        ...
        if params['eval_era'] > 0:
            if era % params['eval_era'] == 0 or era == params['eval_era']:
                ...
                s = evaluation.get_score(
                    model_eval, hist=history, dataset=params['dataset'], inputs=1)
                print s
