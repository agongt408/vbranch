...
def default(input_tensor, num_classes, *layers_spec):
...
def vbranch_default(inputs, final_spec, *layers_spec, branches=1):
    """
    ...
    for i, (filters_list, shared_filters) in enumerate(layers_spec):
        ...
        if i < len(layers_spec) - 1:
            ...
            # Embedding layers
            x = VBL.Dense(layers_spec[-1][0],branches,'fc1',layers_spec[-1][1])(x)
...
def seq_version(input_tensor, num_classes, *layers_spec):
    # NOTE: input_tensor is a single Tensor object
