...
def MergeNet_Drop(base, masks, P_param=1, K_param=1, weights=None,
    ...
    """def fill(dim, shape, branches, n_part, gather_idx, x):
        '''
        Args:
            - l, number of conv filters, e.g, 32 or 128, per layer
        '''
        x = tf.gather_nd(x, tf.cast(gather_idx, tf.int32))
        x = tf.reshape(x, tf.concat([tf.cast(shape, tf.int32)[0][:-1],
            [-1, tf.cast(n_part, tf.int32)[0]]], 0))
        x = tf.tile(x, tf.concat(
            [[1]*4,[tf.cast(branches, tf.int32)[0]]], 0))\
                [:,:,:,:,:tf.cast(dim, tf.int32)[0]]
        return tf.reshape(x, tf.concat(
            [tf.cast(shape, tf.int32)[0][:-1], [-1]], 0))

    def fill_regular(dim, shape, branches, n_part, gather_idx, x):
        return tf.reshape(
            fill(dim, shape, branches, n_part, gather_idx, x),
            [-1,8,4,512]) # tf.cast(shape, tf.int32)[0]

    def fill_concat(shape, branches, pool_part, conv_part, pool_idx,
            conv_idx, pool_final_dim, conv_final_dim, x):
        return tf.reshape(tf.concat([
            fill(512, pool_part, shape, branches, pool_idx,
                pool_final_dim, x),
            fill(32, conv_part, shape, branches, conv_idx,
                conv_final_dim, x)], -1), tf.cast(shape, tf.int32))"""
    ...
    def fill(dim, shape, branches, n_part, gather_idx, x):
        ...
        Args:
            - l, number of conv filters, e.g, 32 or 128, per layer
        '''
        # print 'fill' , shape, gather_idx.shape
        x = tf.gather_nd(x, gather_idx)
        print '1' , x.get_shape().as_list
        x = tf.reshape(x, shape[:-1] + (-1, n_part))
        print '2' , x.get_shape().as_list
        x = tf.tile(x, [1]*4 + [branches])[:,:,:,:,:dim]
        print '3' , x.get_shape().as_list
        return tf.reshape(x, shape[:-1] + [-1])

    def fill_regular(dim, shape, branches, n_part, gather_idx):
        def func(x):
            return tf.reshape(fill(
                dim, shape, branches, n_part, gather_idx, x), shape)
        return func
    ...
    def fill_concat(shape, branches, pool_part, conv_part, pool_idx,
        ...
        def func(x):
            return tf.reshape(tf.concat([
                fill(512, shape, branches, pool_part, pool_idx, x),
                fill(32, shape, branches, conv_part, conv_idx, x)],
                -1), shape)
        return func
            ...
            if outbound.class_name == 'Conv2D' or \
                ...
                for b in range(branches):
                    if masks[l][0].shape[0] <= 512 or \
                        ...
                        # dim
                        n_add, name = Ip(
                            l, n_add, [masks[l][0].shape[0]], name='dim')
                        ib_list[0].append([name, 0])

                        # shape
                        n_add, name = Ip(l, n_add, [shape], name='shape')
                        ib_list[0].append([name, 0])

                        # branches
                        n_add, name = Ip(l, n_add, [branches], name='branches')
                        ib_list[0].append([name, 0])

                        # n_part
                        n_add, name = Ip(l, n_add,
                            [np.where(masks[l][b] == 1)[0].shape[0]],
                            name='n_part')
                        ib_list[0].append([name, 0])

                        # idx
                        gather_idx = get_indices(shape,masks[l][b])
                        n_add, name = Ip(l, n_add, gather_idx, name='idx')
                        ib_list[0].append([name, 0])'''

                        func = fill_regular(
                            dim=masks[copy.deepcopy(l)][0].shape[0],
                            shape=shape, branches=branches,
                            n_part=np.where(masks[copy.deepcopy(l)]\
                                [copy.deepcopy(b)] == 1)[0].shape[0],
                            gather_idx=get_indices(shape,
                                masks[copy.deepcopy(l)]\
                                    [copy.deepcopy(b)]))

                        n_add, name = Lmda(l, n_add,
                            lambda x : copy.deepcopy(func)(x),
                        ...
                        ib_list = [[]]
                        # shape
                        n_add, name = Ip(l, n_add, [shape], tf.int32,
                            name='shape')
                        ib_list[0].append([name, 0])

                        # branches
                        n_add, name = Ip(l, n_add, [branches], tf.int32,
                            name='branches')
                        ib_list[0].append([name, 0])

                        # pool_part
                        n_add, name = Ip(l, n_add,
                            [np.where(masks[l][b][:512] == 1)[0].shape[0]],
                            tf.int32, name='pool_part')
                        ib_list[0].append([name, 0])

                        # conv_part
                        n_add, name = Ip(l, n_add,
                            [np.where(masks[l][b][-32:] == 1)[0].shape[0]],
                            tf.int32, name='conv_part')
                        ib_list[0].append([name, 0])

                        # pool_idx
                        pool_mask = masks[l][b].copy()
                        pool_mask[512:] = 0
                        pool_idx = get_indices(shape, pool_mask)
                        n_add, name = Ip(l, n_add, pool_idx, tf.int32,
                            name='pool_idx')
                        ib_list[0].append([name, 0])
                        ...
                        conv_mask[:512] = 0
                        conv_idx = get_indices(shape, conv_mask)
                        n_add, name = Ip(l, n_add, conv_idx, tf.int32,
                            name='conv_idx')
                        ib_list[0].append([name, 0])
