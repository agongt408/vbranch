...
def MergeNet_Drop(base, masks, P_param=1, K_param=1, weights=None,
                    dense_only=False, tile=False, comp=False):
    '''
    ...
    nth_pool_layer = __get_nth_pool_layer(3, config)

    if dense_only:
        # Branch global average pooling layer
        n_add = __branch(len(init_config.layers) - 5, n_add,
                            first_branch, branches)

        for l in range(len(init_config.layers) - 4, len(init_config.layers) - 1):
            n_add = __branch(l, n_add, first_branch, branches)

        # Gather embeddings from final layer
        n_add = __gather_final(len(init_config.layers)- 1, n_add, branches, masks)

    else:
        for l in range(nth_pool_layer + 1, len(init_config.layers)- 1):
            ...
            if outbound_layer.class_name == 'Concatenate':
                ...
                if config.layers[l + n_add].class_name != 'Concatenate':
                    n_add, first_branch = __branch(l, n_add, first_branch, branches)
                    # print first_branch
...
def DenseNet121Drop(P_param=1, K_param=1, branches=3, overlap=0,
    ...
    l_start = _get_nth_pool_layer(3, base)

    masks = generate_model_masks(base, branches, l_start, overlap)

    # return masks

    regularizers = _get_regularizers(base, l_start, branches, masks=masks)

    batch_norm_list = _get_batch_norm_list(base, l_start, masks=masks)
    ...
    model_drop = MergeNet_Drop(model, masks, P_param, K_param,
                dense_only=dense_only, comp=True)
...
def StackedDenseNet121Drop(P_param=1, K_param=1, branches=3,
    ...
    regularizers = _get_regularizers(
        model, _get_nth_pool_layer(3, model), branches, masks=masks)

    batch_norm_list = _get_batch_norm_list(
        model, _get_nth_pool_layer(3, model), masks=masks)
    '''batch_norm_list = []
    for l in range(_get_nth_pool_layer(3, model), len(model.layers)):
        if model.layers[l].name.find('batch_normalization') > -1:
            batch_norm_list.append(BatchNormalization(
                axis=-1, epsilon=1.1e-5, mask_list=masks[l]))'''
...
def _get_regularizers(model, l_start, branches, overlap=0, masks=None):
    if masks is None:
        masks = generate_model_masks(
            model, branches, l_start=l_start, overlap=overlap)

    regularizers = []

    for l in range(l_start, len(model.layers)):
        try:
...
def _get_batch_norm_list(model, l_start, masks=None):
    batch_norm_list = []
    for l in range(l_start, len(model.layers)):
