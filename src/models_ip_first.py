...
def MergeNet_Drop(base, masks, P_param=1, K_param=1, weights=None,
    ...
    def __branch(l, n_add, first_branch, branches, amend_inbound_nodes=True):
        ...
        for b in range(branches):
            if first_branch < branches:
                n_add, name = __add_mask_layer(l, n_add, masks, b)
                first_branch += 1
            else:
                n_add, name = __add_mask_layer(l, n_add, masks, b)
        ...
        for b in range(branches):
            if first_branch <= 2*branches:
                n_add, name = __add_drop_layer(l, n_add, inbound_name,
                    mask_names[b], 0)
                first_branch += 1
            else:
                n_add, name = __add_drop_layer(l, n_add, inbound_name,
            ...
            if amend_inbound_nodes:
                remove = __amend_inbound_nodes(name, outbound_layer,
                                                remove)
        return n_add, first_branch

    def __add_mask_layer(l, n_add, masks, b):
        ip_op = Input(tensor=K.constant(masks[l][b]))
        name = config.add_input(ip_op, l + n_add)
        n_add += 1
        return n_add, name

    def __add_drop_layer(l, n_add, inbound_name, ip_name, ib_n):
        '''def get_indices(x, mask):
        ...
        def tile_func(x, masks):
            gather_idx = get_indices(masks[0])
            scatter_idx = []
            for b in range(0, len(masks)):
                scatter_idx += __get_indices(masks[b])
            tile = tf.tile(x, len(masks))
            return tf.scatter_nd(tile, tf.cast(scatter_idx, tf.int32))
            '''

        drop_op = Lambda(lambda x : tf.multiply(x[0], x[1]))
        name = config.add_layer('Lambda', drop_op, l + n_add,
                        [[[inbound_name, ib_n], [ip_name, 0]]], [])
        n_add += 1
        return n_add, name

    def __add_shape_layer(l, n_add, shape):
        '''
        Add Input tensor layer with output shape of previous layer
        '''

        shape_op = Input(tensor=K.constant(shape))
        config.add_input(shape_op, l + n_add)
        n_add += 1
        return n_add

    def __amend_inbound_nodes(name, outbound_layer, remove):
    ...
    def __gather_final(l, n_add, branches, masks):
        ...
        for b in range(branches):
            ...
            reshape_op = Input(
                tensor=K.variable([np.where(masks[-1][b] > 0)[0].shape[0]]))
        ...
        if len(masks[l]) > 0:
            ...
            if outbound_layer.class_name == 'Concatenate':
                ...
                if config.layers[l + n_add].class_name != 'Concatenate':
                    ...
                    print config.layers[l + n_add].name, first_branch
                else:
                    remove = True
                    for b in range(branches):
                        l_ib_n = outbound_layer.inbound_nodes.tolist()
